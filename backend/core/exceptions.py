"""
Extended Exceptions for Sprint 2
"""
...
class NotFoundException(GastroCoreException):
    ...
    def __init__(self, resource: str = "Ressource"):
        super().__init__(status_code=status.HTTP_404_NOT_FOUND, detail=f"{resource} nicht gefunden", error_code="NOT_FOUND")
...
class InvalidStatusTransitionException(ValidationException):
    ...
    def __init__(self, current: str, target: str):
        super().__init__(detail=f"Ungültiger Statusübergang: {current} → {target}")


class CapacityExceededException(ValidationException):
    """Capacity exceeded"""
    def __init__(self, detail: str = "Kapazität überschritten"):
        super().__init__(detail=detail)


class ClosedException(ValidationException):
    """Restaurant is closed"""
    def __init__(self, detail: str = "Geschlossen zu dieser Zeit"):
        super().__init__(detail=detail)


class BlacklistedException(ForbiddenException):
    """Guest is blacklisted"""
    def __init__(self):
        super().__init__(detail="Reservierung nicht möglich. Bitte kontaktieren Sie uns telefonisch.")
