"""
GastroCore Table & Staff Import Module
======================================
Persistente Stammdaten: Tische, Kombinationen, Mitarbeiter
...
- POST /api/admin/import/staff              - Excel Upload für Mitarbeiter
- POST /api/admin/seed/from-repo            - Seed aus /seed/ Ordner
- GET  /api/admin/import/staff/status       - Staff Import Status
- GET  /api/admin/import/staff/preview      - Preview der importierten MA
- GET  /api/admin/import/logs               - Import-Protokoll
...
async def get_data_status():
    """GET /api/data-status - System health with collection counts"""
...
async def import_tables(file: UploadFile = File(...), user: dict = Depends(require_admin)):
    """POST /api/admin/import/tables - Import tables from Excel"""
    try:
        import openpyxl
    except ImportError:
        ...
        try:
            ...
            if existing:
                await db.tables.update_one(
                    {"id": existing['id']},
                    {"$set": {"sub_area": subarea, "seats_default": seats, "seats_max": max_seats,
                              "combinable": combinable, "notes": notes, "updated_at": now}}
                )
                updated += 1
            else:
                await db.tables.insert_one({
                    "id": str(uuid.uuid4()), "table_number": table_number, "area": area,
                    "sub_area": subarea, "seats_default": seats, "seats_max": max_seats,
                    "combinable": combinable, "combinable_with": [], "notes": notes,
                    "active": True, "fixed": False, "position_x": None, "position_y": None,
    ...
    await create_audit_log(actor={"id": user['id'], "email": user['email']}, entity="tables",
                           entity_id="bulk_import", action="import",
                           after={"filename": file.filename, "created": created, "updated": updated})
    
    return {"success": errors == 0, "created": created, "updated": updated, "errors": errors,
            "message": f"Import: {created} neu, {updated} aktualisiert, {errors} Fehler"}


# ============== TABLE COMBINATIONS IMPORT ==============
...
async def import_table_combinations(file: UploadFile = File(...), user: dict = Depends(require_admin)):
    """POST /api/admin/import/table-combinations - Import combinations from Excel"""
    try:
        import openpyxl
    except ImportError:
        raise HTTPException(500, "openpyxl not installed")
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        try:
            ...
            combo_data = {"combo_id": combo_id, "area": area, "subarea": subarea,
                          "tables": tables_list, "target_capacity": target_capacity,
                          "notes": notes, "updated_at": now}
            
            if existing:
                await db.table_combinations.update_one({"id": existing['id']}, {"$set": combo_data})
                updated += 1
            else:
            ...
            logger.error(f"Error importing combination: {e}")
            errors += 1
    
    await log_import(user.get('email', 'unknown'), file.filename, 'table_combinations', created, updated, errors)
    await create_audit_log(actor={"id": user['id'], "email": user['email']}, entity="table_combinations",
                           entity_id="bulk_import", action="import",
                           after={"filename": file.filename, "created": created, "updated": updated})
...
async def import_staff(file: UploadFile = File(...), user: dict = Depends(require_admin)):
    """POST /api/admin/import/staff - Import staff from Excel (Upsert)"""
    try:
        import openpyxl
    except ImportError:
        raise HTTPException(500, "openpyxl not installed")
    ...
    # Column mapping
    col_map = {
        'nachname': 'last_name', 'vorname': 'first_name', 'rufname': 'display_name',
        'e-mail': 'email', 'email': 'email', 'telefon': 'phone', 'telefon 2': 'phone_secondary',
        'adresse': 'street', 'stadt': 'city', 'plz': 'zip', 'geburtstag': 'birthday',
        'pers-nr.': 'personnel_number', 'pers-nr': 'personnel_number', 'zeit-pin': 'time_pin',
    }
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        try:
            ...
            data = {db_field: raw_data.get(excel_col) for excel_col, db_field in col_map.items() if excel_col in raw_data}
            ...
            row_hash = create_row_hash({'last_name': last_name, 'first_name': first_name, 'email': email, 'phone': phone})
            ...
            if not existing and personnel_number:
                existing = await db.staff_members.find_one({
                    ...
                    "personnel_number": personnel_number, "archived": {"$ne": True}
                })
            if not existing:
                existing = await db.staff_members.find_one({"external_row_hash": row_hash, "archived": {"$ne": True}})
            
            now = now_iso()
            staff_data = {
                "first_name": first_name, "last_name": last_name, "display_name": display_name,
                "email": email, "phone": phone, "phone_secondary": phone_secondary,
                "birthday": birthday, "personnel_number": personnel_number, "time_pin": time_pin,
                "address": address, "external_source": "excel_import", "external_row_hash": row_hash,
                "updated_at": now,
            }
            
            if existing:
                await db.staff_members.update_one({"id": existing['id']}, {"$set": staff_data})
                updated += 1
            else:
                staff_data.update({
                    "id": str(uuid.uuid4()), "role": "service", "employment_type": "teilzeit",
                    "weekly_hours": 0, "hourly_rate": None, "status": "aktiv",
                    "work_areas": [], "documents": [], "created_at": now, "archived": False
                })
    ...
    await log_import(user.get('email', 'unknown'), file.filename, 'staff_members', imported, updated, errors)
    await create_audit_log(actor={"id": user['id'], "email": user['email']}, entity="staff_members",
                           entity_id="bulk_import", action="import",
                           after={"filename": file.filename, "imported": imported, "updated": updated})
    
    return {
        "success": errors == 0, "imported": imported, "updated": updated, "skipped": skipped,
        "errors": errors, "error_details": error_details[:10],
        "message": f"Import: {imported} neu, {updated} aktualisiert, {errors} Fehler"
    }


@import_router.get("/api/admin/import/staff/preview")
async def preview_staff_import(user: dict = Depends(require_admin)):
    """GET /api/admin/import/staff/preview - Show last imported staff"""
...
async def get_staff_import_status(user: dict = Depends(require_admin)):
    """GET /api/admin/import/staff/status - Show last staff import status"""
    last_import = await db.import_logs.find_one({"collection": "staff_members"}, sort=[("timestamp", -1)])
    total_staff = await db.staff_members.count_documents({"archived": {"$ne": True}})
    imported_staff = await db.staff_members.count_documents({"external_source": "excel_import", "archived": {"$ne": True}})
...
async def seed_from_repo(user: dict = Depends(require_admin)):
    """POST /api/admin/seed/from-repo - Load data from /seed/ folder"""
    try:
        import openpyxl
    except ImportError:
        raise HTTPException(500, "openpyxl not installed")
    ...
    staff_file = SEED_FOLDER / "staff.xlsx"
    
    if not any([tables_file.exists(), combos_file.exists(), staff_file.exists()]):
    ...
    if tables_file.exists():
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            try:
                ...
                if existing:
                    await db.tables.update_one({"id": existing['id']}, {"$set": {
                        "sub_area": subarea, "seats_default": int(data.get('seats', 4)),
                    ...
                    await db.tables.insert_one({
                        ...
                        "seats_max": int(data.get('max_seats', data.get('seats', 4))),
                        "combinable": str(data.get('combinable', 'true')).lower() == 'true',
                        "combinable_with": [], "notes": data.get('notes'), "active": True,
                        "fixed": False, "created_at": now, "updated_at": now, "archived": False
                    })
                    total_created += 1
            except:
                total_errors += 1
    ...
    if combos_file.exists():
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            try:
                ...
                if existing:
                    await db.table_combinations.update_one({"id": existing['id']}, {"$set": {
                        "area": area, "subarea": subarea, "tables": tables_list,
                        "target_capacity": int(data.get('target_capacity', 0)) if data.get('target_capacity') else 0,
                        "notes": data.get('notes'), "updated_at": now
                    }})
                    total_updated += 1
                    ...
                    await db.table_combinations.insert_one({
                        "id": str(uuid.uuid4()), "combo_id": combo_id, "area": area, "subarea": subarea,
                        "tables": tables_list, "target_capacity": int(data.get('target_capacity', 0)) if data.get('target_capacity') else 0,
                        "notes": data.get('notes'), "active": True, "created_at": now,
    ...
    return {"success": total_errors == 0, "created": total_created, "updated": total_updated,
            "errors": total_errors, "message": f"Seed: {total_created} neu, {total_updated} aktualisiert"}
